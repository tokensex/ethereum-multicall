...
var Multicall = /** @class */ (function () {
                ...
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        for (response = 0; response < aggregateResponse.results.length; response++) {
                            ...
                            returnObjectResult = {
                                originalContractCallContext: utils_1.Utils.deepClone(originalContractCallContext),
                                callsReturnContext: [],
                            };
                            for (method = 0; method < contractCallsResults.methodResults.length; method++) {
                                ...
                                if (this._options.tryAggregate && !methodContext.result.success) {
                                    returnObjectResult.callsReturnContext.push(utils_1.Utils.deepClone({
                                        returnValues: [],
                                        decoded: false,
                                ...
                                if (outputTypes && outputTypes.length > 0) {
                                    try {
                                        decodedReturnValues = abi_1.defaultAbiCoder.decode(
    ...
    Multicall.prototype.buildAggregateCallContext = function (contractCallContexts) {
        ...
        for (var contract = 0; contract < contractCallContexts.length; contract++) {
            ...
            var executingInterface = new abi_1.Interface(JSON.stringify(contractContext.abi));
            for (var method = 0; method < contractContext.calls.length; method++) {
                ...
                aggregateCallContext.push({
                    contractContextIndex: utils_1.Utils.deepClone(contract),
                    contractMethodIndex: utils_1.Utils.deepClone(method),
    ...
    Multicall.prototype.findOutputTypesFromAbi = function (abi, methodName) {
        var _a, _b;
        var contract = new ethers_1.ethers.Contract(ethers_1.ZeroAddress, abi);
        methodName = methodName.trim();
        if (contract.interface.getFunction(methodName)) {
            return (_a = contract.interface.getFunction(methodName)) === null || _a === void 0 ? void 0 : _a.outputs;
        }
        for (var i = 0; i < abi.length; i++) {
            if (((_b = abi[i].name) === null || _b === void 0 ? void 0 : _b.trim()) === methodName) {
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        if (!ethersProvider) {
                            ...
                            if (customProvider.nodeUrl) {
                                // @ts-ignore
                                ethersProvider = new ethers_1.JsonRpcProvider(customProvider.nodeUrl);
                            }
                            else {
                                // @ts-ignore
                    ...
                    case 1:
                        network = _a.sent();
                        contract = new ethers_1.Contract(this.getContractBasedOnNetwork(Number(network.chainId)), Multicall.ABI, ethersProvider);
                        overrideOptions = {};
                        if (options.blockNumber) {
                            overrideOptions = __assign(__assign({}, overrideOptions), { blockTag: Number(options.blockNumber) });
                        }
                        if (!this._options.tryAggregate) return [3 /*break*/, 3];
                        return [4 /*yield*/, contract.getFunction("tryBlockAndAggregate").staticCall(false, this.mapCallContextToMatchContractFormat(calls), overrideOptions)];
                    case 2:
                        contractResponse = (_a.sent());
                        return [2 /*return*/, this.buildUpAggregateResponse(contractResponse, calls)];
                    case 3: return [4 /*yield*/, contract.getFunction("aggregate").staticCall(this.mapCallContextToMatchContractFormat(calls), overrideOptions)];
                    case 4:
                        contractResponse = (_a.sent());
                        return [2 /*return*/, this.buildUpAggregateResponse(contractResponse, calls)];
                }
